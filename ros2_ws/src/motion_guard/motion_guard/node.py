...
class MotionGuard(Node):
    def __init__(self, **kwargs):
        ...
        defaults = dict(motion_enabled=False, geometry_confirmed=False, stopping_model_confirmed=False, mount_calibrated=False,
    ...
    def check(self):
        ...
        for topic in (self.cfg['request_topic'],self.cfg['scan_topic'],self.cfg['target_topic'],'/cmd_vel'):
        ...
        target=self.target
        if (target.is_simulated or not target.position_valid or target.status!=TargetState.TRACKING
    ...
    def tick(self):
        ...
        self.status_pub.publish(String(data=json.dumps(dict(mode=self.mode,ready=ready,
