...
函数功能：语音播放函数（带播报完成标志）
**************************************************************************/
void Command::play_audio_feedback_with_flag(const char* audio_file) {
    // 先重置状态，确保之前的完成标志被清除
    g_play_state::reset();
    ...
    if (command) {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("command_recognition"), 
...
函数功能：语音播放函数
**************************************************************************/
void Command::play_audio_feedback(const char* audio_file) {
    if (device_type_ == "M07") {
...
void Command::result_callback(const GoalHandle_::WrappedResult &result)
{
	switch(result.code)
	{
		case rclcpp_action::ResultCode::SUCCEEDED:
			play_audio_feedback("/reach_goal.wav");
			break;
...
void Command::voice_flag_Callback(std_msgs::msg::Int8::SharedPtr msg){
	voice_flag = msg->data;
    if (voice_flag){
        play_audio_feedback("/voice_control.wav");
        cout<<"语音打开成功"<<endl;
    }
}
...
***********************************/
	if (str1 == str2){
		...
		play_audio_feedback("/car_front.wav");
		cout<<"好的：小车前进"<<endl;
	}
/***********************************
指令：小车后退
...
***********************************/
		...
		play_audio_feedback("/car_back.wav");
		cout<<"好的：小车后退"<<endl;
	}
/***********************************
指令：小车左转
...
***********************************/
		...
		play_audio_feedback("/turn_left.wav");
		cout<<"好的：小车左转"<<endl;
	}
/***********************************
指令：小车右转
...
***********************************/
		...
		play_audio_feedback("/turn_right.wav");
		cout<<"好的：小车右转"<<endl;
	}
/***********************************
指令：小车停
...
***********************************/
		...
		motion_msg_pub->publish(motion_);
		play_audio_feedback("/search_voice.wav");
		cout<<"好的：小车寻找声源"<<endl;
	}
/***********************************
指令：小车去I点
...
***********************************/
		...
		play_audio_feedback("/Tracker.wav");
